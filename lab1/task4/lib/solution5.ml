...
let rec find_answer_lazy (Cons((x, y), t)) = if x == 999 then 0 else (if Common.is_palindrome (x * y) then max (x * y) (find_answer_lazy (Lazy.force t)) else (find_answer_lazy (Lazy.force t)))
